...
const OUT = path.join(ROOT, "assets/screenshots/map.svg");
...
for (const t of trips) {
  ...
  if (!dots.has(t.source)) dots.set(t.source, f);
  if (!dots.has(t.destination)) dots.set(t.destination, to);
...
let dotsSvg = "";
for (const [n, p] of dots) {
  dotsSvg += `<circle cx="${p.x}" cy="${p.y}" r="2.5" fill="#888"/><text x="${p.x + 6}" y="${p.y + 3}" font-size="9" fill="#555">${n}</text>`;
}
